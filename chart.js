...
window.drawWeekChart=function(rows,trades,count,asof){
 ...
 const x=i=>58+(i+.5)*620/data.length,y=v=>270-(v-min)/(max-min)*240,w=Math.max(3,620/data.length*.55);
 for(let i=0;i<5;i++){const v=min+(max-min)*i/4;add('line',{x1:55,x2:690,y1:y(v),y2:y(v),stroke:'#d6dacb'});add('text',{x:4,y:y(v)+4,fill:'#586959','font-size':12},f(v));}
 ...
 add('text',{x:55,y:308,fill:'#536250','font-size':12},data[0].firstDate);add('text',{x:690,y:308,fill:'#536250','font-size':12,'text-anchor':'end'},data.at(-1).date);
 const cross=add('g',{'pointer-events':'none',visibility:'hidden'}),vl=add('line',{y1:22,y2:280,stroke:'#344c55','stroke-dasharray':'3 3'},undefined,cross),hl=add('line',{x1:55,x2:690,stroke:'#344c55','stroke-dasharray':'3 3'},undefined,cross),rect=add('rect',{x:0,width:53,height:19,fill:'#263e43'},undefined,cross),pt=add('text',{x:26,fill:'#fff','font-size':11,'text-anchor':'middle'},'',cross),dr=add('rect',{y:283,width:94,height:20,fill:'#263e43'},undefined,cross),dt=add('text',{y:297,fill:'#fff','font-size':11,'text-anchor':'middle'},'',cross);
 let selected=data.length-1;
 function show(index,py=150){selected=Math.max(0,Math.min(data.length-1,index));const d=data[selected],cy=Math.max(25,Math.min(270,py));cross.setAttribute('visibility','visible');vl.setAttribute('x1',x(selected));vl.setAttribute('x2',x(selected));hl.setAttribute('y1',cy);hl.setAttribute('y2',cy);rect.setAttribute('y',cy-10);pt.setAttribute('y',cy+4);pt.textContent=f(min+(270-cy)/240*(max-min));const dx=Math.max(103,Math.min(643,x(selected)));dr.setAttribute('x',dx-47);dt.setAttribute('x',dx);dt.textContent=d.date;
 ...
 const select=e=>{const p=point(e);if(p)show(Math.round((p.x-58)/(620/data.length)-.5),p.y);};
