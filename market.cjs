...
function makeCalendar(cal,prior={}){
 ...
 for(let d=cal.year+'-01-01';d<=cal.year+'-12-31';d=core.addDays(d,1))days[d]=core.weekday(d)<5&&!cal.ranges.some(r=>d>=r.from&&d<=r.to);
 return days;
}
...
function assemble(raw,now,previous){
 const cal=p.calendar(raw.calendar||''),expected=p.expectedClose(cal,now),calendar=makeCalendar(cal,previous?.calendar);
 const events=p.dividends(raw.dividends||'');p.managerCheck(raw.manager||'',events);assertEvents(events);
 ...
 return {snapshot,feeds,status:fresh.length===2?'confirmed':'provisional',audit:{...audit,matchedDays:matched},acceptedHashes:Object.fromEntries(rows.filter(b=>b.date>baseline.asof).map(b=>[b.date,hash(b)]))};
}
module.exports={hash,canonicalize,historicalCheck,makeCalendar,assertEvents,assemble};
